...
//refactored into own function
//bug added
int FeastEffect(int currentPlayer, int choice1, int handPos, struct gameState *state)
{
	int i;
	int temphand[MAX_HAND];
...
int MineEffect(int currentPlayer, int choice1, int choice2, int handPos, struct gameState *state)
{
	...
	if (state->hand[currentPlayer][choice1] < copper || state->hand[currentPlayer][choice1] > gold)	{ return -1; }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case feast:
      ...
      return FeastEffect(currentPlayer, choice1, handPos, state);
			
    case gardens:
      return -1;
			
    case mine:
		return MineEffect(currentPlayer, choice1, choice2, handPos, state);
